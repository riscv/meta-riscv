...
Banana Pi F3 is a low-cost RISC-V 64-bit platform based on the SpacemiT K1 SoC.
...
```
$ kas build /path/to/meta-riscv/kas/bananapi-f3.yml
```
...
* `k1-bananapi-f3.dtb`: device tree for Linux
...
Flash `core-image-full-cmdline-bananapi-f3.rootfs.wic.gz` onto a uSD card (assuming it's accessed through `/dev/sdx`):
...
* [Banana Pi BPI-F3 documentation](https://docs.banana-pi.org/en/BPI-F3/BananaPi_BPI-F3)
